...
async function ensureEbikeImageBucket() {
  const admin = createSupabaseAdminClient();
  const { error: getError } = await admin.storage.getBucket(EBIKE_IMAGE_BUCKET);

  if (!getError) {
    await admin.storage.updateBucket(EBIKE_IMAGE_BUCKET, {
      public: true,
      fileSizeLimit: EBIKE_IMAGE_MAX_BYTES,
      allowedMimeTypes: EBIKE_IMAGE_ACCEPTED_TYPES,
    });
    return admin;
  }
...
export async function POST(request: Request) {
  ...
  if (!adminRole) {
    return NextResponse.json(
      { error: "Nur Administrator:innen duerfen E-Bike-Bilder hochladen." },
      { status: 403 },
    );
  }
