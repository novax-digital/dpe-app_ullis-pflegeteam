...
async function handleCalendarReminderRequest(request: Request) {
  if (!hasSupabaseEnv) {
  ...
  const result = await sendDueCalendarReminders(admin);
...
export async function GET(request: Request) {
  return handleCalendarReminderRequest(request);
}
